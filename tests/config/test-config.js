...
function validateConfig(config, schema, path = '') {
  ...
  for (const [key, expectedType] of Object.entries(schema)) {
    ...
    if (expectedType === 'object' && typeof value === 'object' && !Array.isArray(value)) {
      ...
      errors.push(`Configuration ${currentPath} must be an array, got ${typeof value}`);
    } else if (expectedType !== 'array' && expectedType !== 'object' && typeof value !== expectedType) {
      errors.push(`Configuration ${currentPath} must be ${expectedType}, got ${typeof value}`);
    }
    
    // Additional validation for specific fields
    if (key === 'threshold' && (value < 0 || value > 1)) {
      errors.push(`Configuration ${currentPath} must be between 0 and 1, got ${value}`);
    }
    
    if (key === 'wcagLevel' && Array.isArray(value)) {
      const validLevels = ['wcag2a', 'wcag2aa', 'wcag2aaa'];
      const invalidLevels = value.filter(level => !validLevels.includes(level));
      if (invalidLevels.length > 0) {
        errors.push(`Configuration ${currentPath} contains invalid WCAG levels: ${invalidLevels.join(', ')}`);
      }
    }
  }
...
function loadConfig(configPath = null) {
  ...
  const customConfigPath = configPath || path.join(process.cwd(), 'test-config.json');
  ...
  if (fs.existsSync(customConfigPath)) {
    try {
      ...
      throw new Error(`Failed to load configuration from ${customConfigPath}: ${error.message}`);
    }
  }
  ...
  for (const [section, sectionSchema] of Object.entries(validationSchema)) {
    const sectionErrors = validateConfig(config[section], sectionSchema, section);
